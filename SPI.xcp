...
#define LINE Serial.printf("zyzyzyzy %s:%d \n", __FUNCTION__, __LINE__);
...
//  esp_backtrace_print(5);
  ...
  if (_spi) {
  	LINE;
	Serial.printf("zyzyzyzyA spi_num = %d, sck = %d, miso = %d, mosi = %d, ss = %d\n", 
	  _spi_num,sck,miso,mosi,ss );
    return true;
  }

  Serial.printf("zyzyzyzyB spi_num = %d, sck = %d, miso = %d, mosi = %d, ss = %d\n", 
  	_spi_num,sck,miso,mosi,ss );
